...
var funciones ={
	...
	// Agregando Suma y resta  al objeto funciones
	'suma':['suma','sum','s','+'],
	'resta':['resta','res','r','-'],
...
//agregando las funcion de suma y de resta


var suma = function(val1,val2){
	if(!val1 && !val2 )
	error= 'debe ingresar dos valores para poder ejecutar la operacion';

	resultado =   val1 +' + '+val2+' = '+(parseInt(val1)+parseInt(val2)); //Agregué un parseInt para que no concatene el resultado si no que lo convierta a entero
	if(resultado == 'NaN' || resultado === undefined);
	{
		error = 'debe ingresar un valor numerico  ejemplo 45 ';
	}
...
var resta =function(val1,val2){
	if(!val1 && !val2) error='Debe ingresar dos valores para poder ejecutar la operacion';
	resultado =   val1 +' - '+val2+' = '+(val1-val2);
	if(resultado == 'NaN' || resultado === undefined){
		error = 'debe ingresar un valor numerico  ejemplo 45 ';
	}
